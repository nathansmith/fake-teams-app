...
const hasSupport: boolean = ((): boolean => {
  ...
  try {
    // Set the key/value.
    globalThis.sessionStorage.setItem(key, val);

    // Are we able to read it?
    bool = globalThis.sessionStorage.getItem(key) === val;
...
const cache = hasSupport ? globalThis.sessionStorage : fallback;
...
const set = (key: string, data: IJson): void => {
  ...
  if (isBadKey) {
    // Early exit.
    throw new Error(`Cannot overwrite method: globalThis.sessionStorage.${key}`);
  }
